"""
Experiment 15: Granger-Causal Reward + Barrier Functions (no curriculum)
------------------------------------------------------------------------
Combines the winning Granger causal reward (exp5) with barrier phase-gates (exp4),
WITHOUT curriculum (which was shown to hurt in exp7).

Hypothesis: Barrier B1 hurt exploration in exp4 (4.8% success) because the agent
hadn't yet learned to approach the cube. With Granger reward guiding manipulation,
the barrier penalties may now help *refine* grasp timing rather than block exploration.
...
Reward:
    ...
    ONE-TIME: same as exp5
    CONDITIONAL + TERMINAL: same as exp5
"""
...
class GrangerCausalReward:
    ...
    def compute(self, ee_cube_dist: float) -> float:
        ...
        gripper_delta = np.diff(g)
        cube_delta    = np.linalg.norm(np.diff(c, axis=0), axis=1)
...
class PickPlaceGrangerBarrierEnv(gym.Env):
    ...
    def _compute_reward(self, ee_pos, obj_pos, tray_pos,
        ...
        closure_amount = 1.0 - self._gripper_state
