...
All seeds completed **1.2M steps** in ~8.5 hours each on an NVIDIA RTX A6000.
...
├── fetch_diagnostics.sh           # Download per-step CSVs from tyrone
...
Full per-step `episode_diagnostics.csv` files (~60 MB each) are stored on **tyrone**. Use `fetch_diagnostics.sh` to download them.
...
| **Total GPU Hours** | ~25.5 hours (3 seeds) |
...
*Branch: `results/suction-gripper`*  
*Date: 2026-04-25*  
*Authors: Sher, OpenCode*
